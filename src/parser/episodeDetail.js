...
const episodeDetail = async (episodeId) => {
  ...
  $(".mirrorstream ul").each((i, elem) => {
    ...
    $ul.find("li a").each((j, el) => {
      ...
      if (rawData) {
        try {
          const serverId = obfuscate(rawData);
          serverList.push({
            title: serverTitle,
            serverId,
  ...
  if (defaultBase64) {
    ...
    const postRes = await axios.post(`${BaseUrl}/wp-admin/admin-ajax.php`, formData);
  ...
  $(".flir a").each((i, el) => {
    ...
    if (href && titleAttr) {
      const episodeId = href.split("/").filter(Boolean).pop();
